...
async function welcomeBanner(avatar, name, subject, type) {
    const title = name
    const desc = (type == "welcome" ? "Malvin C Leo ♌️ " : "Welcome  ") + subject
    const background = path.join(__dirname, '../public/menu.png')
...
async function promoteBanner(avatar, name, type) {
    const title = name
    const desc = type == "promote" ? "Malvin C Leo " : "Welcome . Tokugamuchira"
    const background = path.join(__dirname, '../public/menu.png')
    const welcome = await new canvafy.WelcomeLeave()
    .setAvatar(avatar)
    .setBackground("image", background)
